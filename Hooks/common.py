...
TIMEOUT = 100
STATS = ['Stars', 'Ships', 'Economy', '$/Turn', 'Industry', 'Ships/Turn', 'Science', 'Scanning', 'Hyperspace Range',
...
def request_data(config: CommentedMap) -> Dict[str, Any]:
    ...
    for player_data in data['players'].values():
        ...
        for index, title in enumerate(fields):
            if title == '$/Turn':
                temp[STATS[index]] = int(player_data['total_economy'] * 10.0 +
                                         player_data['tech']['banking']['level'] * 75.0)
            elif title == 'Ships/Turn':
                temp[STATS[index]] = int(player_data['total_industry'] *
                                         (player_data['tech']['manufacturing']['level'] + 5.0) / 2.0)
